...
for ax, symb, var in zip([axs[0], axs[1]], ['d', '+'], ['U_along', 'U_fluctuating']):
    ...
    ### Comparison statistical distribution
    train = results_anomalies['5D'].dropna(subset=var).sample(1000, replace=False)
    test = results_anomalies['5D'].dropna(subset=var)
    ...
    exp_loc, exp_scale = expon.fit(abs_u_train, floc=0)
    print('Fitted exponential dist. scale param:', np.round(exp_scale, 3))
...
axs[2].legend(h, l, loc='ur', ncols=1)

l = ['Longitudinal', 'Transverse']
h = [ax.plot([],[], marker=m, color='tab:blue', lw=0) for m in ['d', '+']]
axs[3].legend(h, l, loc='ur', ncols=1)
